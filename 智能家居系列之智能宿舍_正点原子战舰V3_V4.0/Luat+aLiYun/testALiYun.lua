...
local function getDeviceSecret()
	...
	return "4ts1MpFVeMyxpyC3aVhg9LzRLKz8itmo"
end
...
local basedata = {
	...
	id = "1",
	params = {Number = 0,LED_Switch = 1,MQ_2 = 1},
	version = "1.0",
}
...
function publishTest()
	...
	local mq2 = UartTest.a2
	local jsondata
	log.info("r1,r3,a2 = ",num,led,mq2)
	--当阿里云处于连接状态时，才进行发送测试
	--阿里云接收的格式为json
    if sConnected then
		basedata["params"]["Number"] = num
		basedata["params"]["LED_Switch"] = led
		basedata["params"]["MQ_2"] = mq2
        ...
        aLiYun.publish("/sys/a1iXhrN7gRh/868575026608168/thing/event/property/post",jsondata,1,publishTestCb,"publishTest_"..publishCnt)
    end
end
