...
void Smoke_Detect(void);
 ...
 { 
	...
	EXTIX_Init();
	//TIM5_Int_Init(50000,7199);//10Khz的计数频率，计数到15000为5s  
...
//			
			LCD_ShowxNum(20+18*8,170,temp/100,2,16,0);		//显示整数部分
			LCD_ShowxNum(20+21*8,170,temp%100,2,16, 0X80);	//显示小数部分
			if(MQ_INPUT)
			{
				POINT_COLOR=BLUE;//设置字体为蓝色	
...
void Smart_LED1(void)
{
	if(Flag == 0) //自动控制模式
	{
		if(number)//在有人的情况下,进行时间判断和光照判断
		{
			if((calendar.hour > 6) && (calendar.hour < 18)) //时间上的判断处于白天		 6 < hour < 18
			{
				FLAG_H = 0;			
			}
			else						//其他时间处于傍晚
