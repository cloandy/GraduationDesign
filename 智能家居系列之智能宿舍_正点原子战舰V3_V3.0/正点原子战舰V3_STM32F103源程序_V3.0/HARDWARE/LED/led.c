...
void LED_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(DS0_RCC, ENABLE);	 //使能PB 端口时钟

	GPIO_InitStructure.GPIO_Pin = DS0_Pin;				 //LED0-->PB.5 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
	GPIO_Init(DS0_GPIO, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.5
	GPIO_SetBits(DS0_GPIO,DS0_Pin);						 //PB.5 输出高 
}
...
void Smart_Led_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(HC_SR501_1_RCC, ENABLE);	 	 //使能PC端口时钟
	GPIO_InitStructure.GPIO_Pin = HC_SR501_1_Pin;		 //HC-SR501人体红外热释探头输入端（数字量）-->PC.1 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	GPIO_Init(HC_SR501_1_GPIO, &GPIO_InitStructure);	 //根据设定参数初始化GPIOC.1
	
	RCC_APB2PeriphClockCmd(HC_SR501_2_RCC, ENABLE);	 	//使能PC端口时钟
	GPIO_InitStructure.GPIO_Pin = HC_SR501_2_Pin;		//HC-SR501人体红外热释探头输入端（数字量）-->PC.1 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		//上拉输入
	GPIO_Init(HC_SR501_2_GPIO, &GPIO_InitStructure);	//根据设定参数初始化GPIOC.1
	...
	GPIO_InitStructure.GPIO_Pin = LED_Pin;				 //LED0-->PC.3 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 	 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	 //IO口速度为50MHz
	GPIO_Init(LED_GPIO, &GPIO_InitStructure);			 //根据设定参数初始化GPIOC.3
	GPIO_SetBits(LED_GPIO,LED_Pin);						 //PC.3 输出高 
	LED_OUT = 1;
